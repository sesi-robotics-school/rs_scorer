...
enum LABELS_TO_SHOW { NO_LABEL = 0, X_LABEL = 1, Y_LABEL = 2, LEGEND_LABEL = 4 }

enum CHART_TYPE { LINE_CHART, PIE_CHART }
...
func _exit_tree():
	#pie_chart_current_data.queue_free()
	pass
...
func _input(event):
	if event is InputEventMouseMotion:
		...
		if (
			event.position.distance_to(center_point) <= computed_radius
			and current_data != null
			and current_data.size() > 0
		):
			...
			for item_key in current_data[0].keys():
				...
				if computed_angle > initial_angle and computed_angle <= ending_angle:
					...
					hovered_data = {name = item_key, value = item_value}
					break
...
func draw_line_chart():
	...
	for point_data in current_data:
		...
		for key in point_data.sprites:
			...
			point = (
				point_data.sprites[key].sprite.get_position()
				+ texture_size * global_scale / 2.0
			)
			...
			if previous_point.has(key):
				...
				draw_line(
					previous_point[key], point, current_point_color[key].line, current_line_width
				)
		...
		if current_show_label & LABELS_TO_SHOW.X_LABEL:
			...
			draw_string(
				label_font, Vector2(point.x, vertical_line[1].y) - string_decal, label, grid_color
			)
	...
	if current_show_label & LABELS_TO_SHOW.Y_LABEL:
		...
		for ordinate_value in ordinate_values:
			...
			var position = Vector2(
				max(0, 6.0 - label.length()) * 9.5, min_y + max_y - compute_y(ordinate_value)
			)
	...
	if current_show_label & LABELS_TO_SHOW.LEGEND_LABEL:
		...
		for legend_label in current_point_color.keys():
			...
			if current_mouse_over != null and current_mouse_over != legend_label:
				dot_color = Color(
					dot_color.r, dot_color.g, dot_color.b, dot_color.a * COLOR_LINE_RATIO
				)
...
func compute_sprites(points_data):
	...
	for key in points_data.values:
		...
		animation_move_dot(
			sprite,
			end_pos - texture_size * global_scale / 2.0,
			global_scale,
			0.0,
			current_animation_duration
		)

		sprites[key] = {sprite = sprite, value = value}
...
func create_new_point(point_data):
	...
	if chart_type == CHART_TYPE.LINE_CHART:
		...
		current_data.push_back({label = point_data.label, sprites = compute_sprites(point_data)})
	else:
...
func _move_other_sprites(points_data, index):
	if chart_type == CHART_TYPE.LINE_CHART:
		for key in points_data.sprites:
			...
			animation_move_dot(
				sprite, Vector2(x, y) - texture_size * global_scale / 2.0, global_scale, delay
			)
	else:
		var sub_index = 0
...
func animation_move_dot(node, end_pos, end_scale, delay = 0.0, duration = 0.5):
	...
	tween_node.interpolate_method(
		self, "_update_draw", 0.0, 1.0, duration, Tween.TRANS_CIRC, Tween.EASE_OUT, delay
	)
...
func animation_move_arcpolygon(key_value, end_value, delay = 0.0, duration = 0.667):
	...
	tween_node.interpolate_property(
		...
		key_value,
		pie_chart_current_data.get(key_value),
		end_value,
		duration,
		Tween.TRANS_CIRC,
		Tween.EASE_OUT,
	...
	tween_node.interpolate_property(
		pie_chart_current_data,
		radius_key,
		pie_chart_current_data.get(radius_key),
		computed_radius,
		duration,
		Tween.TRANS_CIRC,
		Tween.EASE_OUT,
...
#			result.push_back(value)
	...
	result.push_back(max_value / 10.0 * 1)
	result.push_back(max_value / 10.0 * 2)
	result.push_back(max_value / 10.0 * 3)
	result.push_back(max_value / 10.0 * 4)
	result.push_back(max_value / 10.0 * 5)
	result.push_back(max_value / 10.0 * 6)
	result.push_back(max_value / 10.0 * 7)
	result.push_back(max_value / 10.0 * 8)
	result.push_back(max_value / 10.0 * 9)
